...
- Fixed the memory growth over long uptimes (a five-day process had reached a 2.4GB peak footprint). The Claude Desktop session store walk parsed every `local_*.json` (~1350 files, ~244MB) inside one dispatch block, whose autorelease pool only drains when the block returns, so more than 1GB of parsed trees were alive at once per walk — at launch, on every FSEvents change to a tracked session's store file, and every 2s while a fresh session had no title. Each file is now parsed inside its own `autoreleasepool`, and parse results are cached per path by (mtime, size) so later walks re-parse only changed files: the launch footprint drops from 1.1GB to 25MB, and a repeat walk adds 0.2s of CPU with no footprint change. The 0.1s tick also stopped rewriting labels with the value they already showed, since `NSTextField.stringValue`/`textColor` re-measure the text on every set

## v1.19.0

- A PR chip closed in Claude Desktop now clears the panel's PR icon too. The dismissal is written as `dismissed: true` on the matching `prs[]` entry of the session store the app already walks for session names, so it is picked up on that same walk, persisted as `prDismissed` on the state file, and the row falls back to the plain idle dot. Entries are keyed by URL rather than PR number, since a session can move between repos and the URL is what both the store entry and `gh`'s output always carry. The store stays the source of truth, so a chip restored on the Desktop side brings the icon back. Titles and dismissals resolve independently from the newest file carrying each, so a store file holding only one of them can no longer blank the other; the walks run on a single serial queue because the launch sweep, FSEvents, the untitled-session poll and the menu could otherwise overlap; the launch sweep is scoped to sessions with a recorded PR or a missing name so an idle launch walks no store at all; and a dismissed PR is no longer re-fetched from `gh`

## v1.18.3

- A `Monitor` watch killed by its own timeout left its row on the panel until the session ended. A watch that exits on its own sends the same completion notification as any background command, so the reap clears it; a timed-out one sends a notification naming only the monitor's task id, and matching on that id is what the reap deliberately avoids — per-event progress notifications carry it too, so it would drop a live watch. The hook now records the watch's deadline and sweeps expired rows on every event, which also survives the timeout notification scrolling past the transcript tail before the next hook runs. The deadline is set again when the watch actually arms, so a call left at a permission prompt for longer than its own timeout is not swept the moment it is approved — and if the sweep already took the row while the prompt was open, the response that proves the watch is live puts it back. Every uncertain case rounds the deadline up rather than down — a row swept while its watch is live hides the running work the panel exists to show — so persistent watches get no deadline at all, and rows with none recorded (written before this field existed, or left by a denied persistent call that never reached `PostToolUse`) are bounded by the longest timeout the tool documents so a watch already stuck on the panel clears too
